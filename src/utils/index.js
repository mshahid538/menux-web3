...
export function getAllRestaurants() {
   const result =   Data.restaurants.map((data, index)=>(
        data
    ))
    return result;
}
...
export function getCategoriesByRestaurantId(restaurantId){
    const res = Data.restaurants.filter((data,index)=>(
...
export function getProductsByCategoryId(restaurantId, categoryId) {
  ...
  export function displayAllergiesForRestaurantCategoryAndProduct(restaurantName, category, productName) {
    ...
    if (!menuCategory) {
      console.log(`Category '${category}' not found for restaurant '${restaurantName}'.`);
      return;
    }
    ...
    if (!product) {
      console.log(`Product '${productName}' not found in category '${category}' for restaurant '${restaurantName}'.`);
      return;
    }
  
    console.log(`Allergens for ${restaurantName} - Category: ${category} - Product: ${productName}`);
  ...
  displayAllergiesForRestaurantCategoryAndProduct("Ribeye Steakhouse", "Staters", "Beef Ribs");
  displayAllergiesForRestaurantCategoryAndProduct("Abdul's", "Staters", "Steamed Salmon");
